function [sx_moduli,residual] = MechModuli(load_increment,grainstrain,grainlist)
%  program MechModuli
%
...
%          or (1b)  Neper-generated msh for V4
...
%         and (4)  an estimate of the anisotropic ratio used in defining the initial guess for the set of moduli
%
...
grainstrainave_1 = grainstrain(1,:)*grain_weights(:);
grainstrainave_2 = grainstrain(2,:)*grain_weights(:);
grainstrainave_3 = grainstrain(3,:)*grain_weights(:);

%  
% weight_flag gives option for weighting grain errors in the residual:
%  weight_flag = 0 uses grainlist to mask (to use or ignore) grains (Use =
%  1; ignore = 0); Could be modified to include a relative weight based on confidence of data;
% weight_flag = 1 uses the relative grain volume as a weight
%
...
strainerror = grainstrain - computedstrain;
for icomp =1:1:6
...
message = ['Residual following initial solution using moduli estimated from average strains: ', num2str(residual)];
disp(message)
sx_moduli


computedstrainave_1 = computedstrain(1,:)*grain_weights(:);
computedstrainave_2 = computedstrain(2,:)*grain_weights(:);
computedstrainave_3 = computedstrain(3,:)*grain_weights(:);
% 
