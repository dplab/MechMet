function [bccode,bcvalue,nomstress] = SetBCs_Moduli(m,loadcode,nomstrain,neperversionnumber,varargin)
...
elseif(loadcode==7)
surfspecs = struct('name',   {  'minz',   'maxz'},  ...
		   'bcname', { 'BCZMin', 'BCZMax'} ...
		   ); 
end
%
nnps = size(m.crd, 2);
...
if(loadcode==7)
    ...
    pinpoint = [ min(abs(m.crd(1,:))) min(abs(m.crd(2,:))) min(abs(m.crd(3,:))) ];
    
    offsets = [ m.crd(1,:)-pinpoint(1); m.crd(2,:)-pinpoint(2);  m.crd(3,:)-pinpoint(3) ];
    
    offsetmags = zeros(nnps,1);
    for inps = 1:1:nnps
    offsetmags(inps) = norm(offsets(:,inps));
    end
    
    [minoffset,node] = min(offsetmags);
   ...
   svtotal = cat(1,svtotal,sv);
    ...
    pinpoint = [ max(m.crd(1,:)) min(abs(m.crd(2,:))) min(abs(m.crd(3,:))) ];
